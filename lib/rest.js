...
RestAPI.prototype._update = function(req, res) {
  if (!req.params.id) {
    ...
    this.update(req.params.id, req.body, exports.responseCallback(res));  
  }
}
RestAPI.prototype._remove = function(req, res) {
  if (!req.params.id) {
    ...
    this.remove(req.params.id, exports.responseCallback(res));  
  }
}
...
RestMongoAPI.prototype.get = function(id, cb) {
  var self = this;
  self.db.collection(self.collection).findOne({ _id: ObjectID(id) }, function(err, data) {
...
RestMongoAPI.prototype.update = function(id, data, cb) {
  delete data._id;
  var self = this;
  self.db.collection(self.collection).update({ _id: ObjectID(id) }, data, function(err, _data) {
    data._id = id;
    self._return(err, _data ? data : null, false, cb);
  });
}
RestMongoAPI.prototype.remove = function(id, cb) {
  var self = this;
  self.db.collection(self.collection).remove({ _id: ObjectID(id) }, function(err, data) {
    if (err) {
      cb(500, err.message ? err.message : err);
    } else if (!data) {
      cb(400);
